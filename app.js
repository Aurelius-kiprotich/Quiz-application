...
app.post('/quiz',(req,res) =>{
    ...
    for(let i=1;i<=10; i++){
        ...
        let choice ={
            ...
            correctAswer : answers[i -1],
            score : 0
        }
        if(choice.yourAnswer=== choice.correctAswer){
           choice.score=1
        }
        choices.push(choice)
...
app.post('/edit-profile/:id', uploads.single('picture'), (req, res) => {
    if(req.file){
        let sql = 'UPDATE student SET email = ?, name = ?, gender = ?, dob = ?, picture = ?, contact = ? WHERE s_id = ?'
        ...
        let sql = 'UPDATE student SET email = ?, name = ?, gender = ?, dob = ?, contact = ? WHERE s_id = ?'
